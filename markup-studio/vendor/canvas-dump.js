...
(async()=>{
  ...
  await pg.waitForSelector(`.page-wrap[data-page="${pageN}"] canvas`,{timeout:120000});
  await pg.waitForTimeout(1200);
  const dataUrl=await pg.evaluate((n)=>document.querySelector(`.page-wrap[data-page="${n}"] canvas`).toDataURL("image/png"),pageN);
  fs.writeFileSync(`${SCRATCH}/page-${pageN}@${SC}.png`,Buffer.from(dataUrl.split(",")[1],"base64"));
  console.log(`saved page-${pageN}@${SC}.png`);
