...
function SignedOutAccountPanel() {
  ...
  return (
    <section className="account-panel" aria-label="Account required">
      <div>
        <p className="eyebrow">Account protection</p>
        <h2>Sign in before real scans and paid unlocks.</h2>
        <p>Accounts keep paid Deep Search access attached to you instead of only to a browser or URL.</p>
      </div>
      <SignInButton mode="modal">
        <button className="secondary-button" type="button">
          Sign In
...
export function UnsublyApp({
  ...
  const startingIdentity = initialIdentity || (initialPaid ? "paid-user@example.com" : initialDemoMode ? "demo@example.com" : "");
  ...
  function scanIdentity(value: string) {
    ...
    setActiveFilter("all");
    setFormHint("Demo scan complete. Review each item before removing it.");
  }
      ...
      <main className="main-content">
        ...
        <section className="scan-panel" id="scan" aria-labelledby="scanTitle">
          <div className="scan-copy">
            <h2 id="scanTitle">Start a scan</h2>
            <p>Enter an email or phone number to preview how Unsubly would organize subscriptions. The demo uses sample results so you can test the workflow safely.</p>
          </div>
            ...
            <div>
              ...
              <p>
                {subscriptions.length
                  ? `Showing ${filteredSubscriptions.length} item${filteredSubscriptions.length === 1 ? "" : "s"} for ${identity}.`
          ...
          {!!subscriptions.length && (
            <aside className={`deep-search-panel ${deepSearchUnlocked ? "unlocked" : ""}`} aria-labelledby="deepSearchTitle">
              ...
              {initialDemoMode || deepSearchUnlocked ? (
                ...
                <a className="unlock-button" href={`/api/create-checkout-session?identity=${encodeURIComponent(identity)}`}>
