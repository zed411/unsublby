...
import { createPreviewList } from "../../../../lib/subscriptions";
...
export async function POST() {
  ...
  try {
    ...
    return NextResponse.json({
      source: "gmail",
      subscriptions: subscriptions.length ? subscriptions.slice(0, 6) : createPreviewList(),
      usedFallback: subscriptions.length === 0
    });
  } catch (error) {
